...
| CH-0017 | P1, reopened | REOPENED | CH-0028 exact source `fb0e06e` exposed a boundary-only producer regression: the host attempt timer fired at the 69,999 ms display boundary and the old constructor emitted canonical expiration without proving the 70,000 ms monotonic deadline. The local correction uses one high-resolution monotonic owner, requires `deadlineReached=true`, retains precise elapsed evidence, waits through residual timer granularity, and keeps materially early capped attempts noncanonical. Required-only exact-head verification remains pending. |
