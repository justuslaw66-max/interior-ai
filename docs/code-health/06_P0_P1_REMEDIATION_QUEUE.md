...
| CH-0017 | P1, reopened | REOPENED | Exact-head 8cb7cae Outcome C exposed an undersized bounds phase and unmasked synthetic OAuth metadata; the narrow repository correction is local and still requires an exact-head external run. |
...
- **Scope and rollback:** Revert the Outcome-C follow-up first, then `8cb7cae37d6bb49cd66d61f5523927dc7b64283d`, then the prior CH-0017 follow-ups and `c840c06dc2c5e67f463542292bb7391b0f93d731`. This restores the demonstrated undersized/unmasked path, timing-dependent smoke, unsafe/raw retention, and false-pass/aggregate-only evidence, so it is an emergency rollback rather than an acceptable steady state. Ignored generated evidence is regenerated; ruleset `13671593` remains unchanged and separately administrator-owned.
