...
CH-0001, CH-0012, and CH-0016 are closed for repository-controlled remediation. CH-0017 is **TRUTHFULNESS AND REQUIRED EXTERNAL WORKFLOW VERIFIED — IMPLEMENTATION FROZEN**. CH-0028 is **EXTERNALLY VERIFIED — RESOLVED** at `db346a51718967bd4dc1605b07c0850e02fd08d1`. CH-0029 application remediation is frozen. CH-0030 is the active bounded infrastructure finding; CH-0004 trusted event provenance was not started.
...
- **Severity/status:** P2 performance; **APPLICATION REMEDIATION COMPLETE — IMPLEMENTATION FROZEN; EXTERNAL ENVIRONMENT VERIFICATION PENDING**. No further application, frameloop, GLB lifecycle/cache/readiness/refcount, timeout, retry, or workflow change is authorized under CH-0029. The remaining execution-environment question is owned by CH-0030.
...
- **Title:** Required Chromium runtime smoke suffers renderer/GPU execution contention on the standard CI runner.
- **Severity/status:** P2 test infrastructure; **EXTERNAL VERIFICATION BLOCKED — ISOLATED RUNNER AVAILABILITY/AUTHORIZATION UNCONFIRMED**. This finding begins from authoritative application source `7158b5152336565a1d2b8caa42c2adab108268d2` and does not reopen CH-0017, CH-0028, or CH-0029 application work.
- **Authoritative external evidence:** Required-only run `30805127205` used the standard GitHub-hosted Ubuntu runner, exact source `7158b5152336565a1d2b8caa42c2adab108268d2`, Node 24.13.0/npm 11.6.2, PostgreSQL 15, Playwright 1.60.0, and Chrome for Testing 148.0.7778.96. The required furnished-template identity failed with a measured 8,223 ms unattributed interval; the final `reload-1` `diagnostics-settle-evaluation` callback was requested but never entered. Health/catalog and the pre-smoke truthfulness, migration, authentication, code-quality, and strict-build controls passed. Safe artifact `8852605576` retained exact-source failure provenance with retries 0 and process exit 1.
- **Controlled attribution:** A pristine profile measured callback wait 6,432.420 ms and a 6,926.106 ms renderer-main `RunTask`, of which 238.011 ms was thread CPU and 6,688.095 ms was non-CPU residual. A controlled-pressure profile measured callback wait 6,044.089 ms and a 6,040.329 ms task, of which 198.334 ms was thread CPU and 5,841.995 ms was residual. Both traces follow `RunTask → BeginMainFrame → Commit → DoUpdateLayers → GLES2::ReadPixels → WaitForCmd / Finish / WaitForGetOffset`. Style/layout was approximately 0.257–0.308 ms, paint/raster 0.278–0.286 ms, and garbage collection 0 ms. Maximum Long Animation Frame duration/blocking was 57.7/2.259 ms pristine and 199.4/37.402 ms under pressure; forced style/layout was zero, observer overhead at most 0.2 ms, and trace data loss was absent. Sampled CPU stacks contained no evidence that React/R3F or repeated frame-subscriber JavaScript owned the multi-second interval.
- **Classification:** **CI/browser/GPU execution-environment contention**. The directly measured mechanism is renderer-main waiting for GPU command completion. The trace cannot yet separate host descheduling from GPU-process or software-renderer contention, so pure OS descheduling is not claimed.
- **Test-only harness:** The opt-in E2E profiler uses `page.addInitScript`, a 64-entry/8-script Long Animation Frames projection, fixed-category CDP tracing bounded to 45,000 ms and 256 MiB, test-only User Timing marks, a 64-callback ring, hashed first-party source identities, and a 512 KiB hard limit for sanitized summaries. Trace/video/screenshot capture is off while profiling. Raw trace files are mode 0600, local under ignored `.local/ch0029-runtime-profile/`, deleted before summary publication, and never committed. The retained summary excludes raw URLs and paths, source text, object graphs, screenshots, video, network bodies, cookies, environment values, credentials, and user data. Same-machine strict builds of the working tree and clean detached source `7158b5152336565a1d2b8caa42c2adab108268d2` each produced the identical 26-file `/design` initial-JS graph at 7,096,910 raw / 1,167,760 Brotli bytes: exact delta 0/0.
- **Current execution inventory:** Every checked-in Actions job uses `ubuntu-latest`; no workflow declares `self-hosted`, a larger-runner label, a dedicated browser/performance label, or a runner group. The repository is user-owned rather than organization-owned. The required workflow supports manual dispatch but fixes its jobs to `ubuntu-latest`; the only exact-SHA input belongs to the prohibited Full E2E advisory workflow. Repository/organization runner inventory could not be enumerated because the available GitHub Actions connector has no runner-list endpoint, the local `gh` credential is invalid, and no signed-in browser session was available. An administrator must provide the repository Actions runner list with status/labels/groups, any account/enterprise larger-runner entitlement and label, and any authorized external exact-source environment before execution can proceed.
- **Next requirement:** Run an exact-source required-CI A/B with application source, profiling commit, workflow source, toolchain, PostgreSQL/migrations, Chromium, flags, build/start path, smoke test, canonical timeouts/watchdog/retry count, trace/video configuration, and environment shape held constant. Only runner capacity/isolation may vary. Record runner identity/class, safe CPU/memory and browser/GPU metadata, exact SHAs, wall and renderer CPU durations, non-CPU residual, GLES2/ReadPixels wait, heartbeat/callback/reload timing, all three reload outcomes, and stable/failure evidence. Do not run Full E2E.
- **Outcome contract:** Classify A only if both runtime identities and all three reloads pass, no callback remains requested-but-never-entered, no model-readiness timeout or no-progress watchdog occurs, stable evidence independently verifies, and GPU-command wait/non-CPU residual is materially reduced or no longer blocks correctness; recommend moving only the GPU-sensitive required smoke to that runner class. Classify B if the wait persists under adequate isolation and investigate browser/GPU configuration without speculative application work. Reopen application ownership only if a new trace names a specific first-party callback absent from current profiles. Retain D while no suitable isolated environment is available or authorized; do not weaken required testing.
