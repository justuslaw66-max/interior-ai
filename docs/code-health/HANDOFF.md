...
Status: **REPOSITORY REMEDIATION COMPLETE — EXTERNAL CI ENFORCEMENT VERIFICATION REQUIRED** on `fix/ch-0017-required-test-truthfulness`. Starting SHA: `cea0cabe53742da8b47c1e119d889033351a1fdf`. The local implementation SHA is reported after the single focused commit is created. CH-0001 `62ba966ecb2011e4233c99ce1dcc0641914af008`, CH-0012 `2c9e8b4d2322484a8d80873019d2c3495dd862f5`, and CH-0016 `cbed3550026e2803675f740b49be5fb15f15612b` are ancestors. Entry status, tracked diff, and untracked checks were empty. Before editing, `lsof` resolved the listening Node process to `/Users/justus/Developer/interior-ai`, matching the intended canonical checkout. Nothing was pushed or deployed.
...
The exact-clean-candidate CH-0016 production build/smoke/verify compatibility result is reported in the final Codex response after the implementation commit exists. CH-0016 external GitHub execution and durable artifact retention remain pending; CH-0017 does not reclassify them. External enforcement still requiring dated platform evidence includes GitHub required-check registration, workflow execution/conclusion, immutable source/ref, artifact upload/retention, permissions, and confirmation that advisory E2E is not a required branch-protection check. Vercel, OAuth, scheduler, and database items inherited from CH-0016 remain unverified.

Rollback is one `git revert` of the CH-0017 implementation commit. That removes the fail-closed inventory/report/evidence contract, including the strengthened cabinetry release-evidence schema, and restores known false-pass paths, so it is an emergency rollback rather than an acceptable steady state. Ignored `.local/required-test-evidence/`, `.vercel` reports, and Playwright artifacts are regenerated and are not source-controlled. There is no product/database/persisted-data schema, migration, dependency, catalog-data, production-data, product/UI, push, deployment, or external-setting change to reverse.
