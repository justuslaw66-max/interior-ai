...
- `OpeningDimensionFields` owns width, height, sill, and wall-position inputs for both opening inspectors. The viewport read model retains the complete selected opening and wall bounds; edits use the existing opening metrics action and history transaction. Evidence locks and edit permissions apply to the same shared controls in 2D and 3D.
