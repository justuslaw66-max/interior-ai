...
- Wall sections above and below openings use `openingWallSurfacePanels` to project sill/lintel solids into finish panels. `continuousWallSelection` gives window-bearing wall faces one stable selection identity per side; it removes internal rectangle seams from the outline and retains the perimeter and aperture boundary. Render fragments keep their legacy finish aliases until a whole-wall assignment supersedes them. `surface-material-wall-panels` combines quantities by the resolved assignment, excluding opening area; its `buildRoomWallFinishQuantities` is the single source of default, face and panel wall areas for both the export bill of materials and the editor Surface Summary. `WindowOpeningMesh` owns the aperture-only 3D hit volume and routes selection to the existing opening inspector; hidden cutaway windows cannot intercept clicks. These projections do not rewrite the saved room/opening model or fill openings with wall geometry.
- `lib/room-floor-area` is the single owner of room floor and ceiling area (a custom polygon with at least three points replaces width × depth, an L-shape excludes its drawn corner notch of `L_SHAPE_NOTCH_RATIO` of the width and depth, and holes are subtracted): `getRoomSnapshotFloorAreaSqm` for room snapshots and `getPlanRoomFloorAreaSqm` for 2D plan rooms. Every room-area readout reads it: the export bill of materials, the share, export and PDF room metrics, the editor Surface Summary (`SurfaceRoomSummary.floorAreaSqm`, which also feeds the plan panel Measurements and Floors area, the consumer room setup scale, the floating floor properties panel and the AI brief), the plan room summary, the room and floor inspectors, the 2D active-room HUD and the floor-plan quality room graph.
...
- The central lighting preset owns environment softbox direction through optional `environment.keyPosition` and `environment.fillPosition`, and opposite-side power through `environment.fillIntensity`. Bright & Clear aims asymmetric key and fill softboxes at the scene to distinguish perpendicular wall faces from either side, while neutral ambient fill keeps shaded walls readable. Other presets retain their existing fill ratio and placement. This does not modify surface colors or add scene lights, shadow maps, or per-frame rendering work.
- `useWindowOpeningDrag` owns pointer capture and wall-plane projection for legacy 3D window apertures. `windowOpeningDrag` clamps both coordinates while retaining window dimensions and the original grab point, including mirrored wall directions. `useDesignPageOpeningMoveAction` routes the two coordinates through the existing opening mutation path and respects locked measurements; the canvas interaction controller owns one history transaction and disables camera orbit for the entire gesture. Pointer release, cancellation, lost capture, blur, and unmount all finish the gesture.
- `PlanOpeningDimensionFields` owns the width, height, sill, and wall-position (offset) inputs of the 2D plan inspector; `SelectedOpeningDimensions` owns the kind, width, height, sill, and wall-position inputs (plus the host-repair wall select) of the viewport selection inspector. Both bound the offset to ±(wall span − width)/2, confirm measurement evidence through `FloorPlanPropertyEvidenceControl`, honour evidence locks and edit permissions, and commit through the existing opening metrics action and history transaction. The viewport read model retains the selected opening's metrics, offset, evidence, and wall bounds.
