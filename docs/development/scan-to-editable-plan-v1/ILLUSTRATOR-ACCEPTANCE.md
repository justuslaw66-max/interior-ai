...
Record outcome for each action as PASS, FAIL or NOT_RUN, with input/output hashes and observable object counts. Do not infer editable type from appearance alone. The current export uses PDF Helvetica/WinAnsi and references Helvetica/Arial for SVG; font availability or substitution must be observed, not assumed. Multilingual font support remains incomplete. Proprietary Illustrator layers or PGF data are not promised in app-generated PDFs.
