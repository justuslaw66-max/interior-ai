...
- Diagnostics checkpoint **9c502a6ad34e8c288fd5364d79c3cb5c10019f93**, tree **7553d317ca0585db62c6a4ead9f88f835ef5687f**: retains rejected scale associations/endpoints/counts in private diagnostics, identifies unverified endpoints in review, and saves pipeline stages. Its own normal isolated npm ci and production build passed in E/clean-install-9c502a6-r1. Initial offline install hit a routine Prisma cache-permission error; a normal install with lifecycle access succeeded. Logs preserved. No dependency, tolerance or geometry change.
- Current scoped draft-save batch: consumer corrections can be saved while evidence remains unresolved, using the existing versioned candidate PATCH/validation path. Saving does not confirm or create a design. Recovery copy no longer presumes the user supplied a poor source. The parent component shrank 711→692 lines; quality baselines only lowered. Typecheck, full lint, scan domain checks, quality gate and all20 affected import lifecycle cases (Chromium/WebKit) passed. E/draft-{types-r2,lint-r2,domain-r1,quality-r2,browser-r2}.log. Fresh exact-revision build/browser verification follows this checkpoint.
...
Independent assistant visual audit of all 16 printed dimensions: exact source crops, original/rendered/crop transforms, OCR candidates/orientation, automatically associated endpoints, separately observed tick endpoints, projected/Euclidean lengths and residuals are retained in E/clean-install-21b3354-r1/source-span-audit/{index.html,audit.json,*.png}. The additive source-span-audit-r2 includes complete automatic-host bounds and annotated crops: green observed tick spans, orange rejected hypothesis anchors, red rejected cross-check hosts. These approximate pixel observations are not site measurements or a human pilot. Projected-versus-Euclidean differences are under0.046px; that distinction cannot explain the observed disagreement. The top9260 span is about518px; the bottom chain totaling9260 is about501px. Their same-axis discrepancy remains beyond the frozen3px tolerance using observed ticks, independently of the erroneous automatic associations. No uniform scale is claimed; no image warp performed. Exact reconstruction requires a consistent source or explicitly reviewed measurements/constraints.

A production consumer probe on9c502a6 uploaded the original file through the actual file input, verified the browser File bytes against the source hash, opened review, and set9260 using the observed top span. It retained zero canonical walls. HTTP/persistence were an explicit in-memory harness seeded only with actual adapter output. The browser protocol omitted binary multipart bytes; browser-side hashing resolved that instrumentation issue. A subsequent collapsed-panel selector failure prevented the intended independent cross-check; this is not a completed journey. The probe exposed the missing incomplete-draft save action fixed in the current batch. E/clean-install-9c502a6-r1/consumer-real-draft-r2.
...
- Next: finish exact-revision checks for the incomplete-draft save batch; demonstrate the real scan's consumer draft/calibration/conflict state; complete supported assisted conversion when adequate source evidence is available; finish native Illustrator and independent-plan acceptance. Do not relabel remaining local recognition/UI work as exclusively external.

Runtime provenance last checked: feature3018PID34870 cwd feature worktree; immutable3020PID82067 cwdE/local-preview-dbc0ef4/source; production3032PID18739 cwdE/clean-install-9c502a6-r1/source. Recheck actual lsof before runtime edits. Existing ignored feature dependency overlay remains untouched; installations only in disposable copies.
