...
`lib/floor-plan-imports/vectorizer-evidence.ts` adds a third, server-local evidence source for image uploads and raster PDF pages, next to local OCR and the optional vision classifier. `PythonFloorPlanVectorizerProvider` runs `services/floorplan-vectorizer/floorplan_vectorize.py` and `app_evidence.py` on the rendered page in a private temporary folder (removed afterwards; page bytes never leave the worker) and validates the JSON with zod before anything is used. It is off unless `FLOOR_PLAN_VECTORIZER_ENABLED=1`; `FLOOR_PLAN_VECTORIZER_DIR`, `FLOOR_PLAN_VECTORIZER_PYTHON`, `FLOOR_PLAN_VECTORIZER_TIMEOUT_MS` (default 420000) and `FLOOR_PLAN_VECTORIZER_MAX_PAGES` (default 1) tune it. A failure or timeout leaves the page exactly as the other sources made it (`vectorizer.status: "failed"` in the page diagnostics with the program's last line, and the note "Local vectorizer unavailable: …"). The runtime lives on the machine that runs imports — the worker in `background` mode — as the pinned virtual environment (OpenCV 4, scikit-image, pytesseract) plus the `tesseract` program with English data on `PATH`; `services/floorplan-vectorizer/README.md` ("On the machine that runs imports") lists it, and `<venv>/bin/python3 services/floorplan-vectorizer/doctor.py --run` checks all of it, ending in `RESULT: READY`.
...
A page that prints no dimensions gets no accepted scale: the vectorizer's `estimated_door_leaf` figure (door swings taken as standard leaves) is the fallback. When no printed dimension is confirmed on any page, `solveScale` records that estimate as the page scale with `basis: "assumed_opening_width"` (dimension count 0, confidence 0.3) and the candidate is built under it, so the reviewer sees the rooms at once; the manifest scale carries the basis and the metrics report `scaleAssumed` rather than `scaleSolved`. The `scale_unresolved` issue stays critical and says what the estimate rests on and that every length is approximate until a width is confirmed; the door openings it was made from become `source-scale-estimate` reference marks; and the calibration's primary measurement is the first of them at its estimated width, with the same basis. The scale review panel opens on the amber "Set from an assumed width" badge, still lists the openings, and starts its number as assumed: picking an opening loads its jambs as the two endpoints, and "Update measurement" with a printed or corrected width rescales the whole document to it and clears the assumed state; applying the assumed number unchanged keeps it. The review's own scale dimension is placed on whole-millimetre vertices chosen to measure the entered number exactly, so a rescale from raster jamb pixels does not fail the compile by a rounding millimetre.

A printed dimension that conflicts with the locally confirmed spans does not veto the scale on its own once enough of them agree: when at least six tick-supported spans agree to 2 % (`OUTVOTE_MIN_SUPPORT`), a disagreeing label — whichever reader produced it: a misread digit such as d12's 3650 for 3620 or d11's 2510 for 2570, or a tick taken from the dimension next to it — is left out of the cluster (`outvoteOutliers`) and set aside; the diagnosis notes it ("Printed dimension set aside: ..."), and the review lists it for a look. Fewer agreeing spans keep the old behaviour: one conflict, or one member more than 2 % off, fails the page closed.

Two readers can print two numbers for one printed dimension (d11: the vectorizer's 3730 beside the AI reader's 3700; 2510 beside 2570). Labels at the same place (within 2 % of the page, compatible orientation) with different values are rivals; once a candidate scale exists, `resolveRivalDimensionLabels` lets the ticks decide: the value that sits on the scale over a tick-supported span — its own or a rival's, since each reader's hint may have found different ticks — wins, takes that span as its observation (keeping its own hints), and the others are set aside with a note saying which value the ticks support. A group where no value fits any span is left as it was.
