...
Reload diagnostic resolved: retained synthetic before/after local backup JSON is byte-equivalent (after JSON serialization) and both produce fingerprint a85bc7d7 through the production storedToSnapshot/fingerprintDesignSnapshot functions, matching the post-reload runtime. The failing test had compared a transient pre-save QA marker f1763eee. The assertion now compares the actual saved projection with the reopened runtime, preserving full design verification. No production persistence behavior was changed for this diagnostic. Browser r1 and final failures plus synthetic snapshots retained. Final rerun affects only this new journey; the20 existing cases passed on committedcf91083 and the same production application source.
