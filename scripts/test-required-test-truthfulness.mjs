...

{
  ...
  // GITHUB_ENV from its first step instead, so every later step sees the values
  // the job-level env intended, and the exporter's realpath session transport
  // still takes precedence afterwards.
  const workflowRoot = path.join(process.cwd(), ".github/workflows");
  const declareRootsRun = (sessionSuffix) =>
    [
      ...
      "fi",
      "{",
      "  printf 'CI_AUTH_FIXTURE_RESULT_ROOT=%s\\n' \"$RUNNER_TEMP/interior-ai-auth-results\"",
      `  printf 'CI_AUTH_FIXTURE_SESSION_ROOT=%s\\n' "$RUNNER_TEMP/interior-ai-auth-session-${sessionSuffix}"`,
      "} >> \"$GITHUB_ENV\"",
      "",
    ].join("\n");
  const authJobs = [
  ...
  for (const { file, jobId, sessionSuffix, authStepCount } of authJobs) {
    ...
    assert.equal(
      declare?.name,
      "Declare runner-temp auth fixture roots",
