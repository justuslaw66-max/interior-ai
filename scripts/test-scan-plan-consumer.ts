...
assert.equal(getRoomSnapshotFloorAreaSqm(enclosedConsumer.rooms.find(({ id }) => id === "living")!), 48.56);
assert.equal(getRoomSnapshotFloorAreaSqm(enclosedConsumer.rooms.find(({ id }) => id === "enclosed")!), 7);
