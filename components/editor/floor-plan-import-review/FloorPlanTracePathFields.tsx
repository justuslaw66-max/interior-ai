...
export default function FloorPlanTracePathFields({annotation,document,floorId,onChange,onError,disabled}:{
  ...
  const apply=(next:typeof points)=>{try{const updated=editSourceTrace(document,floorId,annotation.id,next);onChange?.(updated);setUndo({before:document,after:updated.revisionId});onError(null);}catch(c){onError(userFacingErrorMessage(c, "Invalid source point."));}};
