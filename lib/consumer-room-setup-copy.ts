...
export function roomSetupOpeningStatus({
  ...
  if (hasConnectionBlockers) return "A connected room still needs a door. Add one before furnishing.";
  if (!planSettingsReady) return "Checking doors and windows…";
  if (openingCount > 0) return `${openingCount} ${openingCount === 1 ? "door or window" : "doors and windows"} placed.`;
  return "No doors or windows placed yet. Add only the ones that affect fit.";
}
...
export function planPaletteOpeningSummary(planSettingsReady: boolean, openingCount: number): string {
  ...
  return openingCount > 0 ? `${openingCount} ${openingCount === 1 ? "door or window" : "doors and windows"} placed.` : "Doors and windows are optional.";
}
