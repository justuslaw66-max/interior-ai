...
test.describe("00. Runtime smoke", () => {
  test("furnished template remains stable without a render loop", async ({ page }) => {
    ...
    if (await betaStartTemplate.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      await startTemplate.evaluate((control) => (control as HTMLButtonElement).click());
    }
      ...
      .poll(
        () =>
          MODEL_FIXTURES.every(
            ({ modelPath }) =>
              (modelRequestCounts.get(modelPath) ?? 0) >= 1 &&
              (modelResponseCounts.get(modelPath) ?? 0) >= 1
          ),
        { timeout: 60_000 }
      )
      .toBe(true);
    ...
    const settledDiagnosticsAfter = await readModelDiagnostics();
