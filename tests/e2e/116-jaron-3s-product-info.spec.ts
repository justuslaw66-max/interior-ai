...
const JARON_3S_ID = "sofa-real-castlery-jaron-3s";
...
test.describe("116. Jaron 3-Seater Product Info", () => {
  test("API exposes Castlery SG material-specific product info and corrected slim-arm dimensions", async ({ request }) => {
    const endpointReady = await expect
      .poll(async () => {
        const response = await request.get("/api/models/imported");
        if (!response.ok()) return false;
        const body = (await response.json()) as { models?: unknown[] };
        return Array.isArray(body.models) && body.models.some((model) => (model as ImportedModel).id === JARON_3S_ID);
      }, { timeout: 45000 })
      .toBeTruthy()
      .then(() => true)
      .catch(() => false);
    ...
    if (!endpointReady) {
      test.info().annotations.push({
        type: "note",
        description: "Skipping Jaron product-info API assertions because /api/models/imported stayed unavailable",
      });
      return;
    }
    ...
    const model = body.models?.find((entry) => entry.id === JARON_3S_ID);
    expect(model).toBeDefined();

    expect(model?.catalog?.variants?.every((variant) => variant.size_label === "220x115")).toBeTruthy();
    expect(model?.catalog?.variants?.every((variant) => variant.dimensions?.width_cm === 220)).toBeTruthy();
    ...
    expectRows(details?.fabric?.material, [
      ...
      /Care: Fabric sofa/i,
      /Configuration includes: 2 x Power Recliner Armless; 2 x Slim Arm/i,
    ]);
    expectRows(details?.fabric?.dimensions, [
      /Dimension: W220 x D115 x H77cm/i,
      /Product weight: 111\.7kg/i,
      /Reclined product depth: 165cm/i,
    ]);
